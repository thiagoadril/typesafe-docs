---
title: "Guardrails for LLMs - TypeSafe AI"
date: "2026-09-17T20:51:07.420Z"
description: "Screen every message going into and out of an LLM app with one TypeSafe request, describing possible hazards (‘is this a jailbreak attempt?’) and scoring severity (‘how much harm would complying do?’). Threshold the probabilities it hands back and you decide whether to pass, review, block, or route a message to support."
...
image_url: "https://ts-docs.mintlify.app/mintlify-assets/_next/image?url=%2F_mintlify%2Fapi%2Fog%3Fdivision%3DHow-to%26title%3DGuardrails%2Bfor%2BLLMs%26description%3DScreen%2Bevery%2Bmessage%2Bgoing%2Binto%2Band%2Bout%2Bof%2Ban%2BLLM%2Bapp%2Bwith%2Bone%2BTypeSafe%2Brequest%252C%2Bdescribing%2Bpossible%2Bhazards%2B%2528%2527is%2Bthis%2Ba%2Bjailbreak%2Battempt%253F%2527%2529%2Band%2Bscoring%2Bseveri%26theme%3Df0580ae664a0195833f0555d&w=1200&q=100"
image_type: "png"
image_size: 39835
image_height: 630
image_width: 1200
image_size_pretty: "39.8 kB"
word_count: 3050
...
- [Setup](#setup)
- [Load the sample messages](#load-the-sample-messages)
- [Define the guardrails](#define-the-guardrails)
- [Turn the assessment into a decision](#turn-the-assessment-into-a-decision)
- [Screen every message](#screen-every-message)
- [The same probabilities, different decisions](#the-same-probabilities-different-decisions)
- [Look at one decision in full](#look-at-one-decision-in-full)
...
Labs teach most LLMs to refuse a set of unsafe requests, but each lab draws that line somewhere else, and each new version of a model moves it again. You probably want it somewhere else too: stricter in places, and written where you can read it rather than buried in the weights.

Write a system prompt and you have put your rules in exactly the place a jailbreak talks its way past. Put a second LLM in front of the first and you pay a call’s worth of latency and money on every turn, and an attacker can talk that one past too.

Screen each message with one TypeSafe request instead. A battery of `Noul` questions hands you the probability that each hazard holds, and a `Score` question rates how much harm complying would do. “Ignore your instructions” scores as a jailbreak instead of working as one. You then set the thresholds that decide whether a message passes, goes to review, gets blocked, or routes to support.
...
By the end you will have a `guard()` function to put on either side of any LLM call. You edit it in two places: the dict of hazard questions, and the two named routing policies.
...
then set `TYPESAFE_API_KEY`. Every API call is cached in `json_cache.json`, which ships with the cookbook, so re-running replays the published numbers instead of calling the API. Delete that file to run everything live.
...
“Out of bounds” is not one question, so the battery splits it. Four `Noul` questions each return the probability that one hazard criterion holds. Does the message try to override the assistant’s instructions? Does it ask for help with harm or a crime? Does it ask for a diagnosis or a dosage? Does it signal that the sender may hurt themselves? One `Score` question rates how much harm complying would do, on a written scale from “none” to “serious physical harm”.

Both go in the same request, so the whole battery costs one call. The input and output batteries ask the same four things from the two sides: whether the user is asking for it, and whether the reply went ahead and gave it.
...
TypeSafe supplies the assessment; your application owns the decision. Each `Noul` question is compared against two thresholds:
...
The severity `Score` question has a threshold of its own and can turn a review into a block.
...
Every sample message was screened: inputs with the input battery, replies with the output battery. All of them were routed under `strict`.
...
The four actions all appear, and each one is doing something a plain block could not. `melatonin_dose` asks a dosage question mild enough to hand to a human rather than refuse; `self_harm` goes to support instead of being blocked, which is the difference between helping someone and hanging up on them; `novelist_poison` reads as violent and passes anyway, because asking how a detective describes poisoning is not asking to poison anyone. On the output side, `good_refusal` is a reply about breaking into a house that passes, because it is the assistant declining to help.

The input-side `dosage_request` is the one row where the severity `Score` decides the outcome. It asks the same kind of question as `melatonin_dose`, and its `medical_advice` noul would send it to a human on its own. But a severity of 2.02 crosses the block line, so the review becomes a block.
...
[Previous](https://docs.typesafe.ai/cookbooks/citation_check) [SDE cascade Next](https://docs.typesafe.ai/cookbooks/sde_cascade)
...
- [Setup](https://docs.typesafe.ai/cookbooks/llm_guardrails#setup)
- [Load the sample messages](https://docs.typesafe.ai/cookbooks/llm_guardrails#load-the-sample-messages)
- [Define the guardrails](https://docs.typesafe.ai/cookbooks/llm_guardrails#define-the-guardrails)
- [Turn the assessment into a decision](https://docs.typesafe.ai/cookbooks/llm_guardrails#turn-the-assessment-into-a-decision)
- [Screen every message](https://docs.typesafe.ai/cookbooks/llm_guardrails#screen-every-message)
- [The same probabilities, different decisions](https://docs.typesafe.ai/cookbooks/llm_guardrails#the-same-probabilities-different-decisions)
- [Look at one decision in full](https://docs.typesafe.ai/cookbooks/llm_guardrails#look-at-one-decision-in-full)
- [Open it in the playground](https://docs.typesafe.ai/cookbooks/llm_guardrails#open-it-in-the-playground)
