---
title: "Structure recovery - TypeSafe AI"
date: "2026-09-16T21:01:13.487Z"
...
image_url: "https://ts-docs.mintlify.app/mintlify-assets/_next/image?url=%2F_mintlify%2Fapi%2Fog%3Fdivision%3DHow-to%26title%3DStructure%2Brecovery%26description%3DReconstructs%2BMarkdown%2Bfrom%2Bplain%2Btext%2Bthat%2Blost%2Bits%2Bformatting%2Bin%2Btwo%2Brequests%253A%2Bone%2Bstitches%2Bhard-wrapped%2Blines%2Bback%2Btogether%252C%2Bone%2Bclassifies%2Bevery%2Bblock%2B%2528headi%26theme%3Df0580ae664a0195833f0555d&w=1200&q=100"
image_type: "png"
image_size: 39461
image_height: 630
image_width: 1200
...
- [Setup](#setup)
- [The document: a team memo that lost its formatting](#the-document-a-team-memo-that-lost-its-formatting)
- [Pass 1: stitching split sentences](#pass-1-stitching-split-sentences)
- [Pass 2: classifying blocks](#pass-2-classifying-blocks)
- [Rendering](#rendering)
- [Open it in the playground](#open-it-in-the-playground)
- [Appendix](#appendix)
- [Cost and latency](#cost-and-latency)
- [Where the join thresholds come from](#where-the-join-thresholds-come-from)
- [Why the question is “mid-sentence” and not “same paragraph”](#why-the-question-is-mid-sentence-and-not-same-paragraph)
- [The lowest-confidence block](#the-lowest-confidence-block)
...
This cookbook takes plain text whose markup has been stripped (lines hard-wrapped mid-sentence, no heading markers, no list bullets) and reconstructs the structure as Markdown: headings, paragraphs, lists, quotes, code, callouts. The input is a team memo in exactly that state.
...
- **Pass 1, stitch:** one `Noul` question (a yes/no question whose answer is the probability that yes is correct) per adjacent pair of lines, asking whether the line break split a sentence across the two. All the pairs go in a single request, and lines that continue a split sentence get merged back into blocks.
- **Pass 2, classify:** one `Choice` question (pick one option from a list, with a probability for every option) per merged block, choosing among heading, paragraph, list item, quote, code, or callout (a note, tip, or warning set apart from the main text). The blocks only exist once pass 1 has answered, so this is a second request; it also carries companion questions for every block (heading level, step order, callout kind) whose answers are read only when the block’s type makes them relevant.
...
All of the behavior is specified in the pass-2 question criteria: three dicts of one-line descriptions, plus the step question’s true/false criteria inside `classify_questions`. The rest of the code is plumbing around them. The cost and latency numbers are in the appendix: two round trips, 10,211 tokens, 0.8s, \$0.0015 for this memo.
...
Line splitting, blank-line tracking, and id tagging all happen in code; no model is involved. Each line gets a short id (`L014| `); the ids are ordinary text the model reads as part of the state, and questions and answers refer to lines by these ids (the same scheme as the [semantic search cookbook](https://docs.typesafe.ai/cookbooks/semantic_find)).
...
One Noul question per adjacent pair of lines, all in one request; pairs separated by a blank line are skipped. The question is deliberately narrow (“does this line pick up mid-sentence?”), which is close to an objective fact about the text. The appendix covers both the wording choice and how the merge thresholds were derived.
...
The cutoff for merging depends on how the previous line ends. After a dangling line (one with no sentence-ending punctuation), a join probability of 0.2 or above merges the pair; after terminal punctuation (`.` `!` `?` `:` `;`), the cutoff rises to 0.5. The appendix walks through the probabilities behind the two numbers.
...
Code assembles the page from the judgments. Consecutive list items become one list, numbered when the mean of the items’ step probabilities is at least 0.5. That threshold is a group-level decision no single question asked directly.
...
The probabilities land in two separate bands: line breaks that split a sentence score 0.39 and up, breaks the author meant score close to zero. But where to put the cutoff between the bands depends on **how the previous line ends**, a fact code can read directly:
...
- After *terminal* punctuation (a character that ends a sentence or clause: `.` `!` `?` `:` `;`), the cutoff rises to 0.5. The memo’s team list shows why: `L015| The platform team` follows a colon and scores 0.22. That is a low but nonzero “this continues the sentence” signal, and it would clear the 0.2 cutoff and merge the list into the sentence introducing it. No single threshold works for both cases; once code checks the punctuation first, the two bands separate.

## Why the question is “mid-sentence” and not “same paragraph”

The first version of this pipeline asked the obvious question: “are these two lines part of the same paragraph?” It failed in a specific way. A run of short lines under a heading (a list typed without bullets) *is* a paragraph in the loose sense: the lines sit together and share a topic. Asked about paragraphs, the model says yes to every pair, and the stitch pass merges the whole list into one long block.
...
[Previous](https://docs.typesafe.ai/cookbooks/semantic_find) [Function calling Next](https://docs.typesafe.ai/cookbooks/function_calling)
...
- [Setup](https://docs.typesafe.ai/cookbooks/autoformat#setup)
- [The document: a team memo that lost its formatting](https://docs.typesafe.ai/cookbooks/autoformat#the-document-a-team-memo-that-lost-its-formatting)
- [Pass 1: stitching split sentences](https://docs.typesafe.ai/cookbooks/autoformat#pass-1-stitching-split-sentences)
- [Pass 2: classifying blocks](https://docs.typesafe.ai/cookbooks/autoformat#pass-2-classifying-blocks)
- [Rendering](https://docs.typesafe.ai/cookbooks/autoformat#rendering)
- [Open it in the playground](https://docs.typesafe.ai/cookbooks/autoformat#open-it-in-the-playground)
- [Appendix](https://docs.typesafe.ai/cookbooks/autoformat#appendix)
- [Cost and latency](https://docs.typesafe.ai/cookbooks/autoformat#cost-and-latency)
- [Where the join thresholds come from](https://docs.typesafe.ai/cookbooks/autoformat#where-the-join-thresholds-come-from)
- [Why the question is “mid-sentence” and not “same paragraph”](https://docs.typesafe.ai/cookbooks/autoformat#why-the-question-is-%E2%80%9Cmid-sentence%E2%80%9D-and-not-%E2%80%9Csame-paragraph%E2%80%9D)
- [The lowest-confidence block](https://docs.typesafe.ai/cookbooks/autoformat#the-lowest-confidence-block)
