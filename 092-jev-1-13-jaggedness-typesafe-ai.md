...
date: "2026-09-18T00:35:09.879Z"
description: "Jev isn’t perfect. Here are some jagged edges we are aware of with jev-1.13. Many of these will be fixed in later versions."
...
- [The failure modes in detail](#the-failure-modes-in-detail)
- [Literal reading](#literal-reading)
- [Math and Numbers](#math-and-numbers)
  - [Counting](#counting)
  - [Numeric representations](#numeric-representations)
  - [Math using score](#math-using-score)
- [Date and time comparison](#date-and-time-comparison)
- [Indirection](#indirection)
- [Large state full of irrelevant detail](#large-state-full-of-irrelevant-detail)
- [Adversarial content](#adversarial-content)
- [Contradictory instructions and criteria](#contradictory-instructions-and-criteria)
- [Common-sense structural invariants](#common-sense-structural-invariants)
...
`jev-1.13` is fast, calibrated, and good at common-sense judgment but it is not perfect. `jev-1.13` does the best on [System One](https://docs.typesafe.ai/concepts/system-one) tasks. It may struggle with tasks that require additional levels of indirection. It can be quite literal in its understanding. It struggles with tasks that require numeric precision.

## The failure modes in detail

| \#  | Failure mode                                                                                                                          | Do this instead                                                |
| --- | ------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| 1   | [Literal reading](https://docs.typesafe.ai/model-jaggedness/jev-1.13#literal-reading)                                                 | Write the exact condition, criteria for each available options |
| 2   | [Math and Numbers](https://docs.typesafe.ai/model-jaggedness/jev-1.13#math-and-numbers)                                               | Keep the arithmetic in code                                    |
| 3   | [Date and time comparison](https://docs.typesafe.ai/model-jaggedness/jev-1.13#date-and-time-comparison)                               | Extract components; compare in code                            |
| 4   | [Indirection](https://docs.typesafe.ai/model-jaggedness/jev-1.13#indirection)                                                         | Reduce hops; point to the relevant state                       |
| 5   | [Large state full of irrelevant detail](https://docs.typesafe.ai/model-jaggedness/jev-1.13#large-state-full-of-irrelevant-detail)     | Filter first; send only what the question needs                |
| 6   | [Adversarial content](https://docs.typesafe.ai/model-jaggedness/jev-1.13#adversarial-content)                                         | Write precise prompts, and test edge cases before deploying    |
| 7   | [Contradictory instructions and criteria](https://docs.typesafe.ai/model-jaggedness/jev-1.13#contradictory-instructions-and-criteria) | Align the criteria and instruction                             |
| 8   | [Common-sense structural invariants](https://docs.typesafe.ai/model-jaggedness/jev-1.13#common-sense-structural-invariants)           | Ask each decision one way; enforce identities in code          |
| 9   | [Generation](https://docs.typesafe.ai/model-jaggedness/jev-1.13#generation)                                                           | Use a generative model                                         |
...
Every part of a date is a small closed set: twelve months, thirty-one possible days, a bounded range of years. That turns extraction into a [Choice](https://docs.typesafe.ai/primitives/choice) over enumerated options rather than free-form parsing, and it gives you somewhere to put an explicit “not stated” option so a missing part is reported rather than guessed. Code assembles the parts into a real date and owns everything after that, including ordering, duration, offset, and weekday.
...
**Instead:** retrieve and filter in code first, and send only the fields the question needs. When it’s not possible to filter in state, you can use a [Noul](https://docs.typesafe.ai/primitives/noul) to filter for relevance. The [classifying RAG passages cookbook](https://docs.typesafe.ai/cookbooks/classifying_rag_passages) has a worked example.
...
For example, “Is the customer asking for a refund?”, asked as a [Noul](https://docs.typesafe.ai/primitives/noul) and as a yes/no [Choice](https://docs.typesafe.ai/primitives/choice) on the ticket “I’m not happy with the fit. What are my options here?”:
...
**Instead:** when the answer space is bounded, turn extraction into a [Choice](https://docs.typesafe.ai/primitives/choice) over the options rather than asking for the value itself. If you really need to generate text… there are other models for that.
...
[Previous](https://docs.typesafe.ai/legal) [Self-consistency: nouls Next](https://docs.typesafe.ai/cookbooks/consistency_noul_cookbook)
...
- [The failure modes in detail](https://docs.typesafe.ai/model-jaggedness/jev-1.13#the-failure-modes-in-detail)
- [Literal reading](https://docs.typesafe.ai/model-jaggedness/jev-1.13#literal-reading)
- [Math and Numbers](https://docs.typesafe.ai/model-jaggedness/jev-1.13#math-and-numbers)
  - [Counting](https://docs.typesafe.ai/model-jaggedness/jev-1.13#counting)
  - [Numeric representations](https://docs.typesafe.ai/model-jaggedness/jev-1.13#numeric-representations)
  - [Math using score](https://docs.typesafe.ai/model-jaggedness/jev-1.13#math-using-score)
- [Date and time comparison](https://docs.typesafe.ai/model-jaggedness/jev-1.13#date-and-time-comparison)
- [Indirection](https://docs.typesafe.ai/model-jaggedness/jev-1.13#indirection)
- [Large state full of irrelevant detail](https://docs.typesafe.ai/model-jaggedness/jev-1.13#large-state-full-of-irrelevant-detail)
- [Adversarial content](https://docs.typesafe.ai/model-jaggedness/jev-1.13#adversarial-content)
- [Contradictory instructions and criteria](https://docs.typesafe.ai/model-jaggedness/jev-1.13#contradictory-instructions-and-criteria)
- [Common-sense structural invariants](https://docs.typesafe.ai/model-jaggedness/jev-1.13#common-sense-structural-invariants)
- [Generation](https://docs.typesafe.ai/model-jaggedness/jev-1.13#generation)
