---
title: "Parallel questions - TypeSafe AI"
date: "2026-09-17T20:51:07.421Z"
...
- [Setup](#setup)
- [The document: the Wikipedia article on the GDPR](#the-document-the-wikipedia-article-on-the-gdpr)
- [The questions: 8 nouls + 2 choices + 3 scores](#the-questions-8-nouls--2-choices--3-scores)
- [Ask two ways, 5 times each](#ask-two-ways-5-times-each)
- [Batching doesn’t change the answers](#batching-doesnt-change-the-answers)
- [The only difference: cost and speed](#the-only-difference-cost-and-speed)
...
You have one document and N questions about it. You can send one request with all N questions, or N requests with one question each. With TypeSafe the answers come out the same either way: each question is scored on its own against the document, so its answer doesn’t depend on what else is in the request.

To check that, the cookbook asks each question several times both ways - all N in one request, and one question per request - and compares the run-to-run std dev: how far an answer moves from one repeat to the next. Whatever noise a question has, it has under both batching strategies. Batching adds none. Most answers came back identical across all 5 repeats either way, the same value on every call, std dev exactly 0.0.

Cost and speed do change. The document dominates every request. N single-question calls pay for it N times, in N round trips; the batched call pays once. The bigger the document, the nearer that saving comes to a full Nx.

The case here is a regulatory briefing. The document is the Wikipedia article on the GDPR (~54,000 characters, a document-dominated workload where the document is most of every request), and a compliance team wants 13 things checked: 8 `Noul` questions, 2 `Choice` questions, and 3 `Score` questions.
...
`ask()` sends any subset of the questions with the document and reduces each answer to its one tracked number. The document is byte-identical in every call.

Both batching strategies run `RUNS` = 5 times, giving each question 5 answers per strategy, enough to compare the mean (do the two agree?) and the std dev (does batching add noise?). Calls are cached to `json_cache.json`, which ships with the cookbook, so re-rendering is free; delete it to re-run live.
...
[Previous](https://docs.typesafe.ai/cookbooks/consistency_choice_cookbook) [Re-ranking Next](https://docs.typesafe.ai/cookbooks/rerank_typesafe)
...
- [Setup](https://docs.typesafe.ai/cookbooks/parallel_questions#setup)
- [The document: the Wikipedia article on the GDPR](https://docs.typesafe.ai/cookbooks/parallel_questions#the-document-the-wikipedia-article-on-the-gdpr)
- [The questions: 8 nouls + 2 choices + 3 scores](https://docs.typesafe.ai/cookbooks/parallel_questions#the-questions-8-nouls-%2B-2-choices-%2B-3-scores)
- [Ask two ways, 5 times each](https://docs.typesafe.ai/cookbooks/parallel_questions#ask-two-ways-5-times-each)
- [Batching doesn’t change the answers](https://docs.typesafe.ai/cookbooks/parallel_questions#batching-doesn%E2%80%99t-change-the-answers)
- [The only difference: cost and speed](https://docs.typesafe.ai/cookbooks/parallel_questions#the-only-difference-cost-and-speed)
- [Open it in the TypeSafe playground](https://docs.typesafe.ai/cookbooks/parallel_questions#open-it-in-the-typesafe-playground)
