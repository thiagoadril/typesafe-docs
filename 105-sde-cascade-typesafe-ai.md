---
title: "SDE cascade - TypeSafe AI"
date: "2026-09-18T01:07:13.959Z"
...
image_url: "https://ts-docs.mintlify.app/mintlify-assets/_next/image?url=%2F_mintlify%2Fapi%2Fog%3Fdivision%3DExtraction%26title%3DSDE%2Bcascade%26description%3DUses%2Ba%2B2-stage%2Bstructured-data-extraction%2Bcascade%2B%2528mini%2B%25E2%2586%2592%2Bverify%2B%25E2%2586%2592%2Breasoning%2529%2Bto%2Bget%2Bmost%2Bof%2Bthe%2Bquality%2Bof%2Ba%2Bbig%2Breasoning%2Bmodel%2Bat%2Ba%2Bfraction%2Bof%2Bthe%2Bcost.%26theme%3Df0580ae664a0195833f0555d&w=1200&q=100"
image_type: "png"
image_size: 38236
image_height: 630
image_width: 1200
image_size_pretty: "38.2 kB"
word_count: 1191
...
- [Step 2: extract with the mini model (text mode)](#step-2-extract-with-the-mini-model-text-mode)
- [Step 3: verify with TypeSafe](#step-3-verify-with-typesafe)
  - [Run the whole battery over the mini extraction](#run-the-whole-battery-over-the-mini-extraction)
- [Step 4: the escalation gate](#step-4-the-escalation-gate)
- [Step 5: escalate to the reasoning model](#step-5-escalate-to-the-reasoning-model)
- [Step 6: what this looks like on 100 prompts](#step-6-what-this-looks-like-on-100-prompts)
- [Appendix A: what makes a good verifier signal](#appendix-a-what-makes-a-good-verifier-signal)
...
- Overview
  ...
  - the models we use, and their price (\$ per 1M tokens, input / output; standard rates checked September 15, 2026):
    - rung 0 (mini): [`gpt-5.4-mini`](https://developers.openai.com/api/docs/models/gpt-5.4-mini) at \$0.75 / \$4.50
    - rung 1 (reasoning): [`gpt-5.5`](https://developers.openai.com/api/docs/models/gpt-5.5) at \$5.00 / \$30.00 (roughly 7x the mini)
    - verifier: TypeSafe `jev-1.12` at \$0.042 / \$0.00 (output tokens are free; [published Jev pricing](https://typesafe.ai/blog/introducing-system-one-models-and-jev))
- Algorithm
  1.  **Extract** with a cheap/small model.
  2.  **Verify** with **TypeSafe** primitives: a per-field yes/no (“Noul question”) question
...
- This Cookbook
  ...
  - we do *not* use structured outputs, tool calls, or json mode, because:
    - a *schema following* mistake is not the mistake we expect an LLM to make (it’s easy to make synthetic data for this)
...
- The record is **schema-valid** (the line above prints `True`), yet it’s wrong:
  ...
  - but `description` is fabricated: the page never describes a registration date, so mini invents a plausible one. It may parrot the schema’s own example, “Registration opens for the fall semester”, or narrate “…was not found in the document”
  - a JSON-Schema check can’t see this. A cheap model produces confident, schema-satisfying fabrications of this kind, and catching them is the job of a semantic verifier
...
- **The TypeSafe Way: Decomposition**
  ...
  - ![this is the way](https://mintcdn.com/ts-docs/2NirYCl-v96cw05F/cookbooks/sde_cascade/this_is_the_way.jpg?fit=max&amp;auto=format&amp;n=2NirYCl-v96cw05F&amp;q=85&amp;s=10bd7d99dc5f679022bb6763dde57330)
...
- Our results are calibrated: high on the field that is wrong, low on the field that is correct, medium on a field that looks off without being clearly wrong
- This is what a typesafe verifier buys you over a blunt “is this whole thing good?” judge
...
- **These are internal TypeSafe results**, produced with the general method above:
  ...
  - the chart is a historical snapshot; its costs have not been recalculated at the current Jev rate listed above

![internal results: cost/quality frontier over 100 prompts](https://mintcdn.com/ts-docs/2NirYCl-v96cw05F/cookbooks/sde_cascade/pareto_100prompts.png?fit=max&amp;auto=format&amp;n=2NirYCl-v96cw05F&amp;q=85&amp;s=ca6731507e07b501a6627616ba0b767a)
...
[Previous](https://docs.typesafe.ai/cookbooks/llm_guardrails) [Date extraction Next](https://docs.typesafe.ai/cookbooks/date_extraction_cookbook)
...
- [Setup](https://docs.typesafe.ai/cookbooks/sde_cascade#setup)
- [Step 1: the data](https://docs.typesafe.ai/cookbooks/sde_cascade#step-1-the-data)
- [Step 2: extract with the mini model (text mode)](https://docs.typesafe.ai/cookbooks/sde_cascade#step-2-extract-with-the-mini-model-text-mode)
- [Step 3: verify with TypeSafe](https://docs.typesafe.ai/cookbooks/sde_cascade#step-3-verify-with-typesafe)
  - [Run the whole battery over the mini extraction](https://docs.typesafe.ai/cookbooks/sde_cascade#run-the-whole-battery-over-the-mini-extraction)
- [Step 4: the escalation gate](https://docs.typesafe.ai/cookbooks/sde_cascade#step-4-the-escalation-gate)
- [Step 5: escalate to the reasoning model](https://docs.typesafe.ai/cookbooks/sde_cascade#step-5-escalate-to-the-reasoning-model)
- [Step 6: what this looks like on 100 prompts](https://docs.typesafe.ai/cookbooks/sde_cascade#step-6-what-this-looks-like-on-100-prompts)
- [Appendix A: what makes a good verifier signal](https://docs.typesafe.ai/cookbooks/sde_cascade#appendix-a-what-makes-a-good-verifier-signal)
