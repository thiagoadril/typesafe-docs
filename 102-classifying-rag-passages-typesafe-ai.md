---
title: "Classifying RAG passages - TypeSafe AI"
date: "2026-09-16T21:01:13.491Z"
...
image_url: "https://ts-docs.mintlify.app/mintlify-assets/_next/image?url=%2F_mintlify%2Fapi%2Fog%3Fdivision%3DHow-to%26title%3DClassifying%2BRAG%2Bpassages%26description%3DScore%2Beach%2Bretrieved%2Bpassage%2Bwith%2Bone%2BTypeSafe%2Brequest%252C%2Bthen%2Bdecide%2Bin%2Bcode%2Bwhich%2Bones%2Breach%2Bthe%2Banswering%2Bmodel.%2BFor%2Bexample%252C%2Bkeep%2Band%2Bflag%2Bones%2Bthat%2Bcontradic%26theme%3Df0580ae664a0195833f0555d&w=1200&q=100"
image_type: "png"
image_size: 42733
image_height: 630
image_width: 1200
image_size_pretty: "42.7 kB"
word_count: 4535
...
- [Setup](#setup)
- [Load the docs corpus](#load-the-docs-corpus)
- [Retrieve the top passages](#retrieve-the-top-passages)
- [Ask four questions about each passage](#ask-four-questions-about-each-passage)
- [Route each passage in code](#route-each-passage-in-code)
- [Build the prompt from the accepted evidence](#build-the-prompt-from-the-accepted-evidence)
- [Compare the six queries](#compare-the-six-queries)
...
Set `TYPESAFE_API_KEY`, `ANTHROPIC_API_KEY` and `OPENAI_API_KEY`. We use TypeSafe to score each retrieved passage, OpenAI to embed the corpus for the search step, and Claude to write the final answer out of whatever survives the scoring.

None of the three needs a key to reproduce this page. `json_cache.json` ships with the cookbook and replays every recorded call, so a re-render costs nothing. Delete the file to run the pipeline live instead. The numbers here came out of `jev-1.12` and `claude-sonnet-5` on 2026-08-27.
...
The corpus file `corpus.json` holds 81 passages. We copied 80 of them straight from the Supabase auth docs at commit `2440b06`, one passage per heading, verbatim and used under Apache 2.0: <https://github.com/supabase/supabase/tree/2440b06/apps/docs/content/guides/auth>

Each passage carries `id`, `title`, `text` and `source_type`, and every request sends all four. Near-misses fill the set. Rotation, expiry, sessions and signing keys each get their own page, and those pages read alike. Refresh-token rotation and JWT signing-key rotation are different things described in nearly the same words.

We wrote the last one ourselves, `forum-injection`, marked `community_forum`: it reads as an ordinary forum answer until its final paragraph, which is an instruction aimed at the model.

We also wrote two of the six queries to state a premise the docs contradict, so the injection and conflict routes both have something to catch.
...
The premise-contradiction question scores `sessions-01` at 0.92 and sends it to the conflict block. Relevance reads 0.49 and answer evidence 0.51, so those two alone would have dropped it.
...
Nothing reaches the prompt as evidence, which is right for a question built on a false premise. Below, the same table for a query the docs do answer.
...
The injection question is a filter, and only one. A passage that scores under the threshold still reaches the prompt, so the generator prompt has to treat every passage as untrusted text regardless of its score. Nothing here is a security boundary.
...
TypeSafe scores the passages and the routing labels them. An LLM still writes the answer, here `claude-sonnet-5`. Keep accepted and conflicting evidence in separate blocks.
...
The first answer is to the false-premise query, *Refresh tokens expire after 30 days - how do I extend that window?*; the second is to an ordinary question the docs do answer, whose 12 retrieved passages included `forum-injection` and its injected instruction.
...
The first answer arrived with an empty accepted block and one conflicting passage. It opens with “I don’t have sufficient accepted evidence”, names the conflict, and quotes `sessions-01` on refresh tokens never expiring rather than inventing a 30-day setting.

The second had 4 accepted passages and no conflict, and cites all four. Nothing of the injected instruction reaches the text.
...
![output](https://mintcdn.com/ts-docs/5iZnRWRIxyU5JBux/cookbooks/classifying_rag_passages/classifying_rag_passages.executed.1.png?fit=max&amp;auto=format&amp;n=5iZnRWRIxyU5JBux&amp;q=85&amp;s=9257008675e09b97950eaa31f2eec173)

Each bar holds the 12 passages retrieved for one query, 72 in all. At least two thirds of every bar is excluded. Only the two false-premise queries route anything to conflict, and two queries accept nothing at all: the one about a 30-day expiry, and *how are refresh tokens rotated?*
...
[Previous](https://docs.typesafe.ai/cookbooks/entity_alignment) [Double-checking citations Next](https://docs.typesafe.ai/cookbooks/citation_check)
...
- [Setup](https://docs.typesafe.ai/cookbooks/classifying_rag_passages#setup)
- [Load the docs corpus](https://docs.typesafe.ai/cookbooks/classifying_rag_passages#load-the-docs-corpus)
- [Retrieve the top passages](https://docs.typesafe.ai/cookbooks/classifying_rag_passages#retrieve-the-top-passages)
- [Ask four questions about each passage](https://docs.typesafe.ai/cookbooks/classifying_rag_passages#ask-four-questions-about-each-passage)
- [Route each passage in code](https://docs.typesafe.ai/cookbooks/classifying_rag_passages#route-each-passage-in-code)
- [Build the prompt from the accepted evidence](https://docs.typesafe.ai/cookbooks/classifying_rag_passages#build-the-prompt-from-the-accepted-evidence)
- [Compare the six queries](https://docs.typesafe.ai/cookbooks/classifying_rag_passages#compare-the-six-queries)
- [Open it in the playground](https://docs.typesafe.ai/cookbooks/classifying_rag_passages#open-it-in-the-playground)
