---
title: "Date extraction - TypeSafe AI"
...
- [Resolve it in code](#resolve-it-in-code)
- [Run it](#run-it)
- [Confidence to route on](#confidence-to-route-on)
...
The function you build here, `extract_date(document, role)`, takes a document and a phrase naming the date you want, such as “the deadline to return the form”, and hands back a `date` with a confidence. It flags a low-confidence read, and one whose parts do not add up to a date at all, including a date the document never states. The date can be spelled out (“August 14, 2027”) or written relative to today (“tomorrow”, “next Thursday”).

TypeSafe answers `Choice` questions about the date in one call: what kind of date it is, and which month, day, year, or weekday the text names. Code turns those answers into a `date`. The model reads what the text says and never does the calendar math.

The cells below run that function over four short documents, print each date with its confidence, and split the results into the ones code accepts and the ones a person should look at.

![Overview diagram](https://mintcdn.com/ts-docs/2NirYCl-v96cw05F/cookbooks/date_extraction_cookbook/overview.png?fit=max&amp;auto=format&amp;n=2NirYCl-v96cw05F&amp;q=85&amp;s=4d1d1d4d446eefafb556505d9834e7a0)

Overview diagram

*TypeSafe reads how the date is written and which parts the text names. Code turns those answers into a `date`, counting from today when the date is relative, and either accepts it or sends it to review.*
...
Seven `Choice` questions go out in one call. `mode` says how the date is written: `absolute` for a date that names a month, `relative` for one written relative to today, and `none` when the document does not state the date at all.

The other six read the pieces. An absolute date needs `month`, `day`, and `year`. A relative one needs `day_anchor`: today, tomorrow, the day after, or a named weekday. When it names a weekday, `weekday` and `week_offset` say which one and which week. Code reads only the pieces `mode` calls for.

`year` lists one option per year from 1900 to 2050, plus two escapes. `none` means the text states no year and code fills one in. `out_of_range` means the text states a year outside the list, and code flags that instead of guessing. If a list that long bothers you, pull the year-like numbers out of the text first and offer the model only those.
...
The contract states both of its years, so those came off the text. The form states no year, so code filled in 2026: it takes the current year and moves to the next one only when the date is already more than a month past. “today” and “next Thursday” went through the same function as the spelled-out dates.

The kickoff call is the one the form never mentions. There is a date in that form, just not this one, and the note `absolute date incomplete` means `mode` came back `absolute` with no month to go with it. The date came back empty, the confidence reads 0.46, and the row is flagged for a person.
...
[Previous](https://docs.typesafe.ai/cookbooks/sde_cascade) [Pre-parsed value extraction Next](https://docs.typesafe.ai/cookbooks/pre_parsed_value_extraction_cookbook)
...
- [Setup](https://docs.typesafe.ai/cookbooks/date_extraction_cookbook#setup)
- [The questions](https://docs.typesafe.ai/cookbooks/date_extraction_cookbook#the-questions)
- [Resolve it in code](https://docs.typesafe.ai/cookbooks/date_extraction_cookbook#resolve-it-in-code)
- [Run it](https://docs.typesafe.ai/cookbooks/date_extraction_cookbook#run-it)
- [Confidence to route on](https://docs.typesafe.ai/cookbooks/date_extraction_cookbook#confidence-to-route-on)
- [Open it in the TypeSafe playground](https://docs.typesafe.ai/cookbooks/date_extraction_cookbook#open-it-in-the-typesafe-playground)
